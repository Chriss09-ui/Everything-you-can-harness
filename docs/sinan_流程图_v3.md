...
╚═══════════════════════════════════════════════════════════════════════════╝
  ...
  四重嵌套循环：① Sprint  ⊃  ② Session  ⊃  ③ Feature  ⊃  ④ QA/自修复
  （◇=路由判定  ↩=回环及其上限  ⇉=Send 并行扇出）

                    planner  ⇒ 展开成 spec + feature_list（带优先级/依赖）
                       │
 ═ ① Sprint 循环（≤10 轮；第 10 轮后仍未完成 → 抛错强停）════════════════════
                       ▼
                  sprint_plan ──► sprint_negotiate ──◇ 达成一致?
                       ▲                                 │ 否
                       └── ↩回 sprint_plan（协商 ≤3 轮）──┘
                                              是 / 轮次>3 强制放行 │
                                                                  ▼
                                                            sprint_setup
                       │
 ═ ② Session（每轮重置上下文，全靠磁盘 artifact 重建）═══════════════════════
                       ▼
   session_init ⇉ [Sprint1：5 路并行 建 init.sh / feature_list / git / progress；
                    后续 sprint 跳过] ⇉► session_setup
                       │
   session_setup ⇉ [4 路并行 读 pwd / progress / feature_list / git_log] ⇉► sanity_check
                       │
        bug_triage ◄── 否 ──◇ sanity 通过?         （↩回 sanity_check；retry ≤2）
            └────── ↩回 ─────┘            通过 / retry≥2 │ 是
                       │
 ═ ③ Feature 循环（按优先级逐个实现）═══════════════════════════════════════
                       ▼
                  pick_feature ──◇ 还有未完成 feature? ── 否 ──► 去 ④ evaluator_qa
                       ▲                                  │ 是
                       │                                  ▼
                       │          implement_feature ◄── ↩（test 失败，retry ≤2）
                       │                  │
                       │                  ▼
                       │            test_feature ──◇ 通过?
                       │              是 / retry≥2（标 blocked）│
                       │                  ▼
                       └── 是 ── commit_feature ──◇ sprint 内还有未完成?
                                                       │ 否
 ═ ④ QA / 自修复循环 ═══════════════════════════════════════════════════════
                       ▼
        ┌───► evaluator_qa ──◇ overall_pass? ── 是 ──────────────► sprint_complete
        │          │ 否
        │          ▼
        │     evaluator_bugs ──◇ bugs ≥20? ── 是 ──► sprint_complete（放弃修复）
        │          │ 否
        │          ▼
        └── 是 ── generator_fix ──◇ verified 或 fix≥2?   （↩回 evaluator_qa 复评）
                       │ 否
                       └── ↩回 generator_fix（自修复 ≤2 轮）
                       │
 ═ 收尾 ════════════════════════════════════════════════════════════════════
                       ▼
                  sprint_complete ──◇ spec 完成?
                       ├─ 是 ──────────────────────► END ✅
                       └─ 否 ──────────────► ↩回 ① sprint_plan（开新一轮 sprint）
                                       ▼
  ───────────────────────────────────────────────────────────────────────────
