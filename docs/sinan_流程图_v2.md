...
╚═══════════════════════════════════════════════════════════════════════════╝
  ...
  │   作用：评估架构风险，决定是否需要用户人工审批                             │
  │                                                                        │
  │   关键变化：V1 用硬编码规则（挑战分≥4 且 过度设计+失败遗漏非空），         │
  │             V2 改为调用 LLM（shoumen prompt）做综合判断                  │
  │                                                                        │
  │   输入：逆审结果 + 架构包摘要 + 用户需求                                 │
  │                                                                        │
  │   输出（写入 gate_flags）：                                             │
  │     · risk_level（low / medium / high）— ★ LLM 判断结果                │
  │     · reasoning（判断理由）                                             │
  │     · key_concerns（重点关注项）                                        │
  │     · checklist（逐项检查结果）                                        │
  │     · needs_user_approval（risk_level != low 时为 true）                │
  └─────────────────┬──────────────────────────────────┬───────────────────┘
                    │                                  │
        ...
        （架构风险低，自动通过）               （需要用户审阅拍板）
                    ...
                    ▼                                  ▼
  ┌──────────────────────────┐       ┌──────────────────────────────────────┐
  │ 直接进入 final_spec       │       │ ⑫ sinan_approval  ★ 用户交互 2       │
  │ （跳过用户审批）          │       │ 司南展示架构概览 + 逆审评分           │
  │                           │       │                                      │
  │                           │       │ 输入：架构包 + 架构审查               │
  ...
  │   作用：把全程产物压成一份对用户友好的最终交付物                          │
  │                                                                        │
  │   输入：架构包 + 架构审查 + 用户需求表 + framework演进记录               │
  │                                                                        │
  │   输出（两份）：                                                        │
  │     · harness_design_draft.json （结构化的 JSON 设计稿）                 │
  │     · harness_design_final.md   ★ 给用户看的最终交付，含——               │
  │         一、需求确认（目标 / 范围 / 成功标准）                            │
  │         二、架构设计（图描述 / 状态 schema / 阶段顺序 /                   │
  │                       交接协议 / 审批闸门 / 失败恢复）                    │
  │         三、核心模块设计（记忆模块 / 交接协议 / 评估机制）                 │
  │         四、审查摘要（需求审查分 + 架构审查分 + 子代理评审）               │
  │         五、风险摘要表                                                  │
  │         六、Artifact 版本历史                                           │
  └────────────────────────────────────┬───────────────────────────────────┘
                                       ▼
